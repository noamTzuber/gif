...
def decode_test1(input_compressed, code_size):
    ...
    current_item = table[compressed_data[0]]
    decompressed_data += current_item

    for item in compressed_data[1:]:
        if item in table:
            next_item = table[item]
        else:
            next_item = current_item + current_item[:code_size]
        decompressed_data += next_item
        if current_item:
            table[convert_int_to_bstring(table_size, code_size)] = current_item + next_item[:code_size]
            table_size += 1
...
def decode_test2(input_compressed, code_size):
    compressed_string = input_compressed.decode()
    compressed_data = [compressed_string[i:i + code_size] for i in range(0, len(compressed_string), code_size)]
    decompressed_data = ""
    table_size = 2
    ...
    for code in compressed_data:
        if not (code in table):
            table[code] = string + (string[:code_size])
        decompressed_data += table[code]
        if not (len(string) == 0):
            table[convert_int_to_bstring(table_size, code_size)] = string + (table[code][:code_size])
            table_size += 1
        string = table[code]
...
if __name__ == '__main__':
    c, f = encode_test(b"100010")
    print("bytes =", type(c), "codesize = ", type(f))
    print(decode_test2(c, f))
